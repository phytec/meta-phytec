...
if [ -n "$SER_P0_ENT" ] && [ -n "$DESER_ENT" ] && [ "$PORT" = "0" ] ; then
	echo ""
	echo "  Configure Routing for phyCAM-L Port 0 on CSI2"
...
if [ -n "$SER_P1_ENT" ] && [ -n "$DESER_ENT" ] && [ "$PORT" = "1" ] ; then
	echo ""
	echo "  Configure Routing for phyCAM-L Port 1 on CSI2"
