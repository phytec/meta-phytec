...
media-ctl -l "'mxc-mipi-csi2.0':4->'mxc_isi.0':0[1]" ${VERBOSE}
media-ctl -l "'mxc_isi.0':12->'mxc_isi.0.capture':0[1]" ${VERBOSE}

if [ -L /dev/phycam-deserializer-csi1 ] && [ -L /dev/phycam-serializer-csi1 ] ; then
	DESERIALIZER="$(cat /sys/class/video4linux/$(readlink /dev/phycam-deserializer-csi1)/name)"
	SERIALIZER="$(cat /sys/class/video4linux/$(readlink /dev/phycam-serializer-csi1)/name)"
	media-ctl -l "'${SERIALIZER}':1->'${DESERIALIZER}':0[1]" ${VERBOSE}
fi

media-ctl -V "'${CAM_ENT}':0[fmt:${FMT}/${RES} ${OFFSET}/${FRES}]" ${VERBOSE}
media-ctl -V "'mxc-mipi-csi2.0':0 [fmt:${FMT}/${RES} field:none]" ${VERBOSE}
