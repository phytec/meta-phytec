...
if [ -n "$SER_P1_ENT" ] && [ -n "$DESER_ENT" ] && [ "$PORT" = "1" ] ; then
	echo "  phyCAM-L Port 1 Serializer on CSI2"
...
echo "  XBar CSI2:"
if [ -n "$SER_P1_ENT" ] && [ -n "$DESER_ENT" ] && [ "$PORT" = "1" ] ; then
	echo "   $MC -V \"'${XBAR_ENT}':3/1[fmt:${FMT}/${RES} field:none]\""
	$MC -V "'${XBAR_ENT}':3/1[fmt:${FMT}/${RES} field:none]" ${VERBOSE}
else
	echo "   $MC -V \"'${XBAR_ENT}':3/0[fmt:${FMT}/${RES} field:none]\""
	$MC -V "'${XBAR_ENT}':3/0[fmt:${FMT}/${RES} field:none]" ${VERBOSE}
fi
echo ""
