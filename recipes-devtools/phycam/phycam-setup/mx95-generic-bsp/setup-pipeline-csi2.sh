...
if [ -n "$SER_P0_ENT" ] && [ -n "$SER_P1_ENT" ]; then
	CAM_P0="/dev/cam-csi2-port0"
	CAM_P1="/dev/cam-csi2-port1"
...
CAP_ENT="mxc_isi.4.capture"
ISI_ENT="mxc_isi.4"
ISI_ENT2="mxc_isi.6"
XBAR_ENT="crossbar"
FMT_ENT="4ac10000.syscon:formatter@120"
MIPI_ENT="csidev-4ad40000.csi"
...
if [ -n "$DESER_ENT" ] ; then
	echo "  Enabling phyCAM-L on CSI2"
...
if [ -n "$SER_P1_ENT" ] && [ -n "$DESER_ENT" ] && [ "$PORT" = "1" ] ; then
	if [ -n "$CAM_2_ENT" ]; then
		media_set_format "${XBAR_ENT}" 3 0 "${CAM_2_FMT} field:none"
	fi
	media_set_format "${XBAR_ENT}" 3 1 "${FMT}/${RES} field:none"
else
	if [ -n "$CAM_2_ENT" ]; then
		media_set_format "${XBAR_ENT}" 3 1 "${CAM_2_FMT} field:none"
	fi
	media_set_format "${XBAR_ENT}" 3 0 "${FMT}/${RES} field:none"
fi
echo ""
